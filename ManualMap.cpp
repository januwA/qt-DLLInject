        ...
        while (pImportDescr->Name)
        {
            ...
            for (; *pThunkRef; ++pThunkRef, ++pFuncRef)
            {
                ...
                else
                {
                    ...
                    *pFuncRef = _GetProcAddress(hDll, (char*)pImport->Name);
                }
            }
            pImportDescr++;
        }
...
bool ManualMap(QWidget* context, HANDLE hProcess, string dllpath)
{

    // 1. 将dll读取到buffer
    if (!GetFileAttributesA(dllpath.c_str()))
    {
        QMessageBox::information(context, NULL, "GetFileAttributesA error.");
        return false;
    }
    ...
    if (File.fail())
    {
        QMessageBox::information(context, NULL, "File error.");
        return false;
    }
    ...
    {
        QMessageBox::information(context, NULL, "is not PE file.");
        delete[] dllBuffer;
        return false;
    }
    ...
    if ( !newmem )
    {
        ...
        if (!newmem)
        {
            QMessageBox::information(context, NULL, "虚拟内存分配失败");
            delete[] dllBuffer;
            return false;
        }
    }
    ...
    // 将dllbuffer写入newmem, SizeOfHeaders 是dos头到以一个节的大小
    WriteProcessMemory(hProcess, newmem, dllBuffer, opt_header->SizeOfHeaders, 0);

    // 整个dllbuffer被写入虚拟内存中了
    delete[] dllBuffer;

    // 申请一块虚拟内存 存shellcode
    BYTE* pShellcode = (BYTE*)VirtualAllocEx(hProcess, 0, 0x1000, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if(!pShellcode)
    {
        QMessageBox::information(context, NULL, "为shellcode申请虚拟内存失败.");
        return false;
    }
    if(!WriteProcessMemory(hProcess, pShellcode, (LPCVOID)Shellcode, 0x1000, 0))
    {
        QMessageBox::information(context, NULL, "shellcode写入虚拟内存失败.");
        return false;
    }

    // 创建远程线程，执行shellcode，然后传入参数地址
    HANDLE hThread = CreateRemoteThread(hProcess, 0, 0,
        (LPTHREAD_START_ROUTINE)pShellcode, newmem, 0, 0);
    if (!hThread)
    {
        QMessageBox::information(context, NULL, "远程线程创建失败");
